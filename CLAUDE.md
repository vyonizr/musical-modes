...
**Embeddable progression widget:** `?embed=1` in the URL makes `main.tsx` mount `src/components/EmbedWidget.tsx` instead of `App` — same single Vite entry/bundle, no router. `EmbedWidget` reads `progression` (a `PROGRESSIONS[].id`), `key`, `acc`, and an optional `bpm` override from the query string (`key`/`acc` use the same encoding as the main app's URL state, see above) and renders one `TableContent` row per *distinct* mode in the progression's `steps` — so a modal-mixture progression (`double-plagal`, `backdoor-vamp`) shows multiple mode rows in the embed, with the currently-playing cell highlighted the same way the main table does. Chord-sequencing playback (cancel-and-release-on-retrigger, per-step timing off `bpm`) lives in `src/utils/usePlayProgression.ts`, shared by both `App.tsx` and `EmbedWidget.tsx` — this was extracted from `App.tsx` specifically so the embed wouldn't reimplement it; don't duplicate it a third time. `ProgressionsPanel`'s "Copy embed code" button (`handleCopyEmbed` in `App.tsx`) builds the `<iframe>` snippet from the main app's *currently selected* key/accidental plus the progression's own `bpm`, and sizes the iframe via `computeEmbedHeight(modeCount)` in `src/utils/embed.ts`. See `docs/sdd/specs/2026-07-11-embeddable-progression-widget.md` for the full rationale (including why BPM is fixed at embed-creation time rather than reader-adjustable, and why the backlink carries `key`/`acc`/`modes` params through to the main app).
