...
`ProgressionsPanel`'s "Copy embed code" button (`handleCopyEmbed` in `App.tsx`) builds the `<iframe>` snippet from the main app's *currently selected* key/accidental plus the progression's own `bpm`, and sizes the iframe via `computeEmbedHeight(modeCount)` in `src/utils/embed.ts`. See `docs/sdd/specs/2026-07-11-embeddable-progression-widget.md` and `docs/sdd/specs/2026-07-11-embed-widget-custom-progressions.md` for the full rationale.
...
- **`BPM_DEFAULT` (120, in `progressions.ts`) has two other fallbacks that must stay in sync by hand:** the `?? 120` in `usePlayProgression.ts`'s duration calc, and the `?? 120` in `handleCopyEmbed`'s embed `src` in `App.tsx`. These already drifted out of sync once (fixed in commit `19ab10e`) — grep for `?? 120` if you ever change the default.
